...
namespace Agora.Spaces
{
    public class Portal : NetworkBehaviour
    {
        ...
        {
            if (label.TryGetComponent(out Mirror.Examples.AdditiveLevels.LookAtMainCamera lookAtMainCamera))
        ...
        IEnumerator SendPlayerToNewScene(GameObject player)
        {
            if (player.TryGetComponent(out NetworkIdentity identity))
            {
                ...
                var scene = SceneManager.GetSceneByPath(destinationScene);
                //Debug.LogWarning("Moving to scene:" + scene.name);
                // Move player to new subscene.
                SceneManager.MoveGameObjectToScene(player, scene);
                GameApplication.Instance.NotifySceneChange(scene.name);
